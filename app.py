...
st.set_page_config(page_title="QuLab: Lab 36: Integrating with Excel/BI Tools", layout="wide")
st.sidebar.image("https://www.quantuniversity.com/assets/img/logo5.jpg")
st.sidebar.divider()
st.title("QuLab: Lab 36: Integrating with Excel/BI Tools")
st.divider()
...
# Page: ESG Governance Report
elif page == 'ESG Governance Report':
    ...
    def highlight_esg_comp(s):
        val = str(s['esg_in_compensation']).lower()
        if val == 'yes':
            return ['background-color: #C6EFCE'] * len(s)
        elif val == 'no':
            return ['background-color: #FFC7CE'] * len(s)
        return [''] * len(s)
...
elif page == 'Compliance & Model Info':
